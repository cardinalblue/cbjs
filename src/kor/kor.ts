...
export class Point implements Elementable<number> {
  ...
  rotateP(angle: number, c: Point = Point.ZERO) {
...
export function $S(width: number, height: number) {
...
export class Rect {
  ...
  get center() {
    return $P(this.origin.x + this.size.width / 2,
...
export function $R(origin: Point, size: Size) {
